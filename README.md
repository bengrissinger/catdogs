...
project's edge node:
```bash
kubectl get svc edge-catdogs -n $MY_NAMESPACE
```
...
```bash
kubectl create secret generic greymatter-catdogs-edge-certs \
...
It is up to you to get them there! Following the pattern defined in the `k8s/manifests.yaml` edge-catdogs Deployment 
